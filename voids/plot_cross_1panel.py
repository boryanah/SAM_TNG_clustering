...
bin_centers = np.load("data_cross/bin_cents.npy")
cross_mean = np.load("data_cross/cross_true_mean.npy")
cross_err = np.load("data_cross/cross_true_error.npy")
...
for i in range(len(opts)):
    ...
    color = colors[i]
    opt = opts[i]
    cross_opt_mean = np.load("data_cross/cross_"+opt+"_mean.npy")
    cross_opt_err = np.load("data_cross/cross_"+opt+"_error.npy")

    rat_opt_mean = np.load("data_cross/rat_cross_"+opt+"_mean.npy")
    rat_opt_err = np.load("data_cross/rat_cross_"+opt+"_error.npy")
    ...
    if i == 0:
        ...
        ax_scatter.plot(bin_centers,cross_mean*bin_centers**power,linewidth=1.,color='black',label="TNG300")
    ...
    ax_scatter.set_ylabel(r"$\xi_{gv,\mathrm{model}}(r)$",fontsize=fontsize+6)
    ...
    ax_histx.set_ylabel(r'$\xi_{gv,\mathrm{model}}/\xi_{gv,\mathrm{TNG300}}$',fontsize=fontsize+6)
    ax_histx.set_ylim([0.65,1.35])

plt.savefig("all_cross_voids.pdf")
plt.show()
